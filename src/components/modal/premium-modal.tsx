...
import { TPremiumType, premiumSchema } from "./form-model";
...
export const PremiumModal = () => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<TPremiumType>({ resolver: zodResolver(premiumSchema) });
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    await fetchCustom("api/form", {
      method: "POST",
      data: JSON.stringify({ ...data, type: EFormType.PREMIUM }),
    });
    ...
    // Close Modal
    (
      document.getElementById("modal--premium-modal") as HTMLFormElement
    ).close();
  ...
  return (
    <dialog id="modal--premium-modal" className="modal">
